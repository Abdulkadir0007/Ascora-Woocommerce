...
jQuery(function ($) {
    ...
    $.post(
      ...
      function (res) {
        ...
        if ($("body").hasClass("wishlist-page")) {
          $(".ascora-wishlist-item[data-id='" + pid + "']").fadeOut(
            250,
            function () {
              $(this).remove();
  ...
  $(document).on("click", ".wl-remove", function (e) {
    ...
    $item.addClass("removing").find(".loading-gif").removeClass("hidden"); // visual flag
    ...
    $.post(
      ...
      function (res) {
        ...
        $item.fadeOut(250, function () {
          $(this).remove();
          $(".wishlist-count, .ascora-wishlist-icon .count").text(
            res.data.count
          );
          if (!$(".ascora-wishlist-item").length) location.reload();
        });
...
(function ($) {
  ...
  var ajax = {
    wc: window.ascora_wc || {},
    ajax_url:
      window.ascora_wc && ascora_wc.ajax_url
        ? ascora_wc.ajax_url
  ...
  function updateMainImageByUrl(imgUrl) {
    ...
    if (state.gallerySwiper) {
      ...
      // fallback single img
      $(".woocommerce-product-gallery__image img, .asp-main-img")
        .first()
        .attr("src", imgUrl);
    }
  }
  ...
  function bindGlobalEvents() {
    ...
    $(document).on("click", "#ascora-cart-trigger", function (e) {
      ...
      $("#ascora-mini-cart-wrapper").html(
        '<div class="ascora-loading">Loading...</div>'
      );
      refreshMiniCart();
    });
    ...
    $(document).on(
      "click",
      ".ascora-slide-close, #ascora-slide-overlay",
      function () {
    ...
    $(document).on("click", "body", function (e) {
      if (
        !$(e.target).closest("#ascora-slide-cart, #ascora-cart-trigger").length
      ) {
        $("#ascora-slide-cart, #ascora-slide-overlay").removeClass("active");
      }
    });
    ...
    $(document).on("click", ".mini-remove", function (e) {
      ...
      $.post(
        ajax.wc.ajax_url,
        { action: "ascora_remove_cart_item", cart_item_key: key },
        function (res) {
    ...
    $(document).on("click", ".mini-qty.plus, .mini-qty.minus", function () {
      ...
      $.post(
        ajax.wc.ajax_url,
        { action: "ascora_update_qty", cart_item_key: key, change: change },
        function (res) {
          $("#ascora-mini-cart-wrapper").html(res);
          updateCartCount();
        }
    ...
    $(document).on("click", ".quick-view", function (e) {
      ...
      $.post(
        ajax.wc.ajax_url,
        { action: "ascora_quick_view", product_id: pid },
        function (response) {
    ...
    $(document).on(
      "click",
      ".ascora-card .action-btn.quick-view",
      function (e) {
        e.preventDefault();
        var pid = $(this).data("product-id");
    ...
    $(document).on(
      "click",
      ".ascora-card .action-btn.compare-btn",
      function (e) {
        e.preventDefault();
        var pid = $(this).data("product-id");
    ...
    jQuery(function ($) {
      ...
      $(document).on(
        "click",
        ".ascora-shop-color-variations .color-dot",
        function (e) {
          e.preventDefault();
      ...
      $(document).on(
        ...
        function (e) {
          ...
          var $form = $dot.closest(
            "form.variations, .product, .asp-right, body"
          );
          var $colorSelect = $form
  ...
  function updateQVImage($dot, imgUrl) {
    ...
    const $mainSlide = $qvWrapper
      .find(".qv-swiper .swiper-slide.qv-gallery-item")
      .first();
  ...
  $(document).on(
    "click",
    ".ascora-qv-wrapper .ascora-shop-color-variations .color-dot",
    function (e) {
      e.preventDefault();
  ...
  function initSingleProduct(context) {
    ...
    $(context)
      ...
      .each(function () {
        ...
        $wrap
          .off("mousemove.asp zoom.leave")
          .on("mousemove.asp", function (e) {
